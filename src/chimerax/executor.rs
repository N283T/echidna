...
impl ChimeraXExecutor {
    ...
    /// Execute Python code directly using ChimeraX's -c flag.
    /// This implies --nogui and --silent.
    pub fn run_python_code(&self, code: &str) -> Result<Output> {
        self.log_execution("ChimeraX -c '...'");
...
"#;
        let output = self.run_python_code(python_code)?;
        let stdout = String::from_utf8_lossy(&output.stdout);
