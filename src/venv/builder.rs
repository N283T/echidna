...
//!
//! Creates a virtual environment directory structure that references ChimeraX's Python,
//! allowing IDEs and type checkers to resolve ChimeraX imports correctly.
//!
...
/// Backend used for venv creation.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum VenvBackend {
    /// Use uv (faster, preferred)
    Uv,
    /// Use manual symlinks (fallback)
    Manual,
}

impl std::fmt::Display for VenvBackend {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            VenvBackend::Uv => write!(f, "uv"),
            VenvBackend::Manual => write!(f, "manual symlinks"),
        }
    }
}
...
impl VenvBuilder {
    ...
    pub fn build(&self) -> Result<()> {
        match self.backend {
            VenvBackend::Uv => self.build_with_uv(),
            VenvBackend::Manual => self.build_manual(),
        }
    }

    /// Build venv using uv.
    fn build_with_uv(&self) -> Result<()> {
        let mut args = vec![
            "venv".to_string(),
            "--python".to_string(),
            self.python_info.executable.clone(),
        ...
        if !output.status.success() {
            ...
            return Err(EchidnaError::VenvError(format!(
                "uv venv failed: {}",
                stderr.trim()
            )));
        }
    ...
    /// Build venv using manual symlinks.
    fn build_manual(&self) -> Result<()> {
    ...
    fn create_windows_links(&self) -> Result<()> {
        ...
        eprintln!(
            "  Warning: Python executable was copied (not linked). \
             Recreate venv after ChimeraX updates."
        );
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn test_venv_backend_display() {
        assert_eq!(format!("{}", VenvBackend::Uv), "uv");
        assert_eq!(format!("{}", VenvBackend::Manual), "manual symlinks");
    }
}
